...
it('renders deterministic accessible SVG with a transparent root', () => { const result = render('fixture', 'flowchart TD\nA[Hello] --> B((World))'); expect(result.svg).toContain('<svg id="fixture"'); expect(result.svg).toContain('aria-label='); expect(result.svg).not.toContain('background:#ffffff'); expect(result.svg).not.toMatch(/<svg[^>]*><rect/); expect(result.svg).toMatchSnapshot(); });
