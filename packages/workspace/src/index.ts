...
export interface NodeGeometry { x?: number; y?: number; width?: number; height?: number; positionLocked?: boolean; positioning?: NodePositioning }
...
export type Selection = { type: 'node'; id: string } | { type: 'edge'; id: string } | undefined;
...
export function normalizeNodeGeometry(input: NodeGeometry): NodeGeometry {
  const result: NodeGeometry = { positioning: input.positioning ?? 'auto', positionLocked: input.positionLocked ?? false };
...
export class WorkspaceController {
  private workspace?: Workspace; private readonly listeners = new Set<(workspace: Workspace) => void>(); private unsubscribe?: () => void; private saveTimer?: ReturnType<typeof setTimeout>;
  ...
  async load(): Promise<Workspace> { this.workspace = await this.repository.get(this.view.workspaceId) ?? createWorkspace(this.view.workspaceId); this.transport.connect(this.view.workspaceId); this.unsubscribe = this.transport.subscribe((event) => this.receive(event)); this.emit(); return this.workspace; }
  get current(): Workspace | undefined { return this.workspace; }
  subscribe(listener: (workspace: Workspace) => void): () => void { this.listeners.add(listener); if (this.workspace) listener(this.workspace); return () => this.listeners.delete(listener); }
  update(recipe: (workspace: Workspace) => Workspace): Workspace { if (!this.workspace) throw new Error('Workspace is not loaded.'); const now = Date.now(); this.workspace = normalizeWorkspace({ ...recipe(this.workspace), id: this.workspace.id, revision: this.workspace.revision + 1, updatedAt: now }); this.emit(); this.scheduleSave(); this.transport.publish({ type: 'workspace.updated', eventId: `${this.view.viewId}:${this.workspace.revision}`, workspaceId: this.workspace.id, originViewId: this.view.viewId, revision: this.workspace.revision, workspace: this.workspace }); return this.workspace; }
  ...
  private receive(event: WorkspaceEvent): void { if (!this.workspace || event.workspaceId !== this.workspace.id || event.originViewId === this.view.viewId || event.revision <= this.workspace.revision) return; this.workspace = normalizeWorkspace(event.workspace); this.emit(); this.scheduleSave(); }
  private emit(): void { if (this.workspace) for (const listener of this.listeners) listener(this.workspace); }
