...
export function snapBounds(moving: Bounds, targets: SnapTarget[], scale: number, tolerancePx = 6, grid?: { enabled: boolean; spacing: number }): SnapResult {
...
export function snapResize(original: Bounds, intended: Bounds, handle: ResizeHandle, targets: SnapTarget[], scale: number, tolerancePx = 6, grid?: { enabled: boolean; spacing: number }): SnapResult {
  ...
  const objectX = activeResizeCandidate(intended, handle, targets, 'x', tolerance); const objectY = activeResizeCandidate(intended, handle, targets, 'y', tolerance);
  ...
  const gridX = activeX !== undefined && grid?.enabled && grid.spacing > 0 ? Math.round(activeX / grid.spacing) * grid.spacing - activeX : undefined; const gridY = activeY !== undefined && grid?.enabled && grid.spacing > 0 ? Math.round(activeY / grid.spacing) * grid.spacing - activeY : undefined;
