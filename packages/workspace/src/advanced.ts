...
export function applyNodeResize(workspace: Workspace, id: string, result: ResizeResult): Workspace {
  ...
  return { ...workspace, layout: { ...workspace.layout, mode: workspace.layout.mode === 'auto' ? 'hybrid' : workspace.layout.mode, nodes: { ...workspace.layout.nodes, [id]: { ...current, ...result, positioning: 'manual', sizing: 'manual' } } } };
}
type Candidate = { adjustment: number; kind: 'edge' | 'center'; position: number; targetId: string };
function axisCandidates(moving: Bounds, targets: Array<{ id: string; bounds: Bounds }>, axis: GuideAxis): Candidate[] {
...
export function snapBounds(moving: Bounds, targets: Array<{ id: string; bounds: Bounds }>, scale: number, tolerancePx = 6, grid?: { enabled: boolean; spacing: number }): SnapResult {
  ...
  const gridX = grid?.enabled && grid.spacing > 0 ? Math.round(moving.x / grid.spacing) * grid.spacing - moving.x : undefined; const gridY = grid?.enabled && grid.spacing > 0 ? Math.round(moving.y / grid.spacing) * grid.spacing - moving.y : undefined;
...
export function insertEdgeWaypoint(workspace: Workspace, edgeId: string, point: Point, segmentIndex: number): Workspace { const current = workspace.layout.edges[edgeId] ?? { routing: 'auto' as const }; if (current.routingLocked || !finite(point.x, point.y) || !Number.isInteger(segmentIndex) || segmentIndex < 0) return workspace; const waypoints = [...(current.waypoints ?? [])]; waypoints.splice(Math.min(segmentIndex, waypoints.length), 0, point); return { ...workspace, layout: { ...workspace.layout, edges: { ...workspace.layout.edges, [edgeId]: { ...current, routing: 'manual', waypoints } } } }; }
