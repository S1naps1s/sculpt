...
In editable Canvas views the generated SVG is strictly visual: it does not receive pointer events, and native text selection and drag behavior are disabled within the canvas. Pointer-down and canvas focus clear stale DOM ranges so focus changes between windows cannot leak browser selection into an editing gesture. Source textareas and Inspector controls remain normally interactive, while Preview and Presentation keep the generated SVG available for future document interactions.
...
Resize snapping has a separate solver: only the edge named by the active resize handle participates as the moving source, while target edges and centers remain available. Grid snapping likewise uses the moving edge coordinate, preventing stationary aligned edges or the changing node center from defeating an intended resize snap. For mixed locked selections, movement bounds include only movable nodes; selected locked nodes remain stationary snap references. A fully locked selection does not open a gesture transaction.

Direct edge double-click requires an actual overlay edge hit, projects the pointer onto the nearest polyline segment, and inserts the waypoint at that route position. Manual routes preserve waypoint order. Directly editing Auto or Straight starts a fresh Manual route from the visible route, intentionally discarding inactive waypoint metadata; Orthogonal and Bézier direct insertion no-op. Projection and route ordering are pure geometry operations with stable segment-index tie breaking. Routing locks reject insertion, movement, deletion, and routing changes. Orthogonal waypoint movement retains horizontal/vertical elbows; Phase 4 deliberately does not introduce a full routing engine.
